...
const Card = ({ children, className = "", noPadding = false }) => (
    <div className={`bg-white rounded-3xl shadow-soft hover:shadow-soft-hover transition-shadow duration-300 ${noPadding ? '' : 'p-6'} ${className}`}>
        {children}
    </div>
);

const NestedCard = ({ title, children, className = "", rightAction = null, mainShadow = false }) => (
    <div className={`bg-[#eef2f6] rounded-[28px] p-2 border border-white/60 transition-shadow duration-300 flex flex-col ${mainShadow ? 'shadow-main-card' : 'shadow-soft'} ${className}`}>
        {title && (
            <div className="flex justify-between items-center px-4 pt-2 pb-3 shrink-0">
                <h3 className="font-semibold text-slate-700 font-sans text-sm tracking-wide">{title}</h3>
                {rightAction && <div className="text-sm">{rightAction}</div>}
            </div>
        )}
        <div className="bg-white rounded-[20px] px-5 pb-5 pt-3 shadow-sm flex-1 min-h-0 flex flex-col overflow-hidden relative">
            {children}
        </div>
    </div>
);
...
const TwistyBarChart = () => {
    ...
    return (
        <div className="relative h-[180px] w-full mt-4 flex-1">
            <div className="absolute top-0 right-0 bg-slate-800 text-white text-xs px-3 py-1 rounded-full shadow-lg z-10 translate-x-2 -translate-y-2 font-serif">
                $2,567
            </div>
                ...
                {data.map((item, i) => {
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center group relative cursor-pointer justify-end h-full pt-4">
                            <div className="opacity-0 group-hover:opacity-100 absolute top-0 bg-slate-800 text-white text-xs px-2 py-1 rounded transition-opacity pointer-events-none z-30 font-serif">
                                {item.value}%
                            </div>
...
const AGENTS_DATA = [
    {
        ...
        states: ['TX', 'FL', 'GA'], contracts: 12, policies: 87,  status: 'Active',   joined: 'Jan 10 2024',
    ...
    },
    {
        ...
        states: ['CA', 'AZ', 'NV'], contracts: 8, policies: 63, status: 'Active', joined: 'Feb 22 2024',
...
const NewsCard = ({ t }) => (
    <div className="flex flex-col h-full justify-start">
        <div className="py-4 border-b border-slate-100 cursor-pointer group">
            <div className="flex justify-between items-start mb-1">
                ...
                <span className="text-[10px] text-slate-400 font-serif">{t.today}</span>
            </div>
            <p className="text-sm font-medium text-slate-700">{t.newCarrierDesc}</p>
        </div>
        <div className="py-4 border-b border-slate-100 cursor-pointer group flex items-center justify-between">
            <div>
            ...
            <div className="text-right">
                <p className="text-sm font-bold text-slate-700 font-serif">Mar 14</p>
                <p className="text-[10px] text-slate-400 font-serif">8:30 AM</p>
            </div>
        </div>
    </div>
);
...
const DashboardView = ({ navigateTo }) => {
    ...
    <div className="fade-in space-y-8">
        <header className="mb-6">
            <h1 className="text-5xl md:text-6xl font-bold text-slate-800 tracking-tight font-sans">{t.welcome}</h1>
            <p className="text-slate-500 mt-3 font-serif text-lg">{t.subtitle}</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 flex flex-col">
                <NestedCard title={isAgency ? t.agencyRevenue : t.weeklySales} mainShadow={true} className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                        <div className="text-5xl font-bold text-slate-800 font-serif">
                            {isAgency ? '$148,500.00' : '$4,250.00'}
                        </div>
                        <Badge variant="green">{isAgency ? '+23%' : '+15%'}</Badge>
                    </div>
                    <TwistyBarChart />
                </NestedCard>
            </div>

            {/* Card 2 — Policy Status (agent) / Agent Sales (agency) */}
                        ...
                        <div className="flex-1 flex items-end justify-between gap-2 h-40 pt-4">
                            {[
                                ...
                                <div key={i} className="flex flex-col items-center flex-1 group h-full justify-end">
                                    <div className="w-full relative bg-slate-50 rounded-t-xl flex items-end justify-center h-full overflow-hidden transition-all duration-300 group-hover:-translate-y-1">
                                        <div className={`w-full ${step.color} rounded-t-xl transition-all duration-500`} style={{height:`${step.val}%`}}></div>
                                        <span className="absolute bottom-2 text-white font-bold text-sm drop-shadow-md font-serif">{step.val}</span>
                                    </div>
                        ...
                        <div className="flex gap-3 flex-1">
                            ...
                            <div className="flex-1">
                                ...
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                                        ...
                                        <span className="text-xs font-bold bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full">{a.count}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex-1">
                                ...
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                                        <span className="text-sm font-medium text-slate-700">{a.name}</span>
                                        <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{a.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {/* Card 3 — Policy Notifications (agent) / News (agency) */}
            <div className="lg:col-span-3 flex flex-col">
                {!isAgency ? (
                    <NestedCard title={t.policyNotif} className="flex-1">
                        <div className="flex flex-col h-full justify-start">
                            {[
                                { title: 'Aetna Health',      desc: t.signPending,    time: t.hoursAgo,  dot: 'bg-orange-400', aiFlag: false },
                                { title: 'Ambetter Medicare', desc: t.docsReady,      time: t.today,     dot: 'bg-green-400',  aiFlag: false },
                                { title: 'Cigna Life',        desc: t.needsAttention, time: t.yesterday, dot: 'bg-red-400',    aiFlag: true  },
                            ].map((item, i) => (
                                <div key={i} className="flex items-start gap-3 py-4 border-b border-slate-100 last:border-0 group cursor-pointer">
                                    ...
                                    <span className="text-[10px] text-slate-400 font-serif">{item.time}</span>
                                </div>
                            ))}
                        </div>
                    </NestedCard>
                ) : (
                    <NestedCard title={t.news} className="flex-1">
                        <NewsCard t={t} />
                    </NestedCard>
                )}
            </div>

            {/* Card 4 — Contract Notifications (both modes) */}
            <div className="lg:col-span-5 flex flex-col">
                        ...
                        {CONTRACT_NOTIFS.map((notif, i) => (
                            <div key={i} className="flex items-center justify-between py-4 border-b border-slate-100 last:border-0 group cursor-pointer" onClick={() => navigateTo('contract-detail')}>
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center font-bold text-slate-700 font-serif text-xs group-hover:bg-brand-50 group-hover:text-brand-600 transition-colors shrink-0">
                                        {notif.id}
                                    </div>
                                    <div>
            ...
            {/* Card 5 — Contract Status & Licenses stats (both modes, different numbers) */}
            <div className="lg:col-span-4 flex flex-col justify-center gap-12 px-4 py-6">
                <div>
                    <h3 className="text-xs font-bold text-slate-400 mb-6 font-sans uppercase tracking-widest border-b border-slate-200/60 pb-2">{t.contractStatus}</h3>
                    <div className="flex text-center divide-x divide-slate-200">
                        <div onClick={() => navigateTo('contracts', { tab: 'Approved' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-brand-600 font-serif drop-shadow-sm">{contractStats.approved}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.approved}</div>
                        </div>
                        <div onClick={() => navigateTo('contracts', { tab: 'In Progress' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-yellow-500 font-serif drop-shadow-sm">{contractStats.inProgress}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.inProgress}</div>
                        </div>
                        <div onClick={() => navigateTo('contracts', { tab: 'Rejected' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-red-500 font-serif drop-shadow-sm">{contractStats.rejected}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.rejected}</div>
                        </div>
                    </div>
                </div>
                <div>
                    <h3 className="text-xs font-bold text-slate-400 mb-6 font-sans uppercase tracking-widest border-b border-slate-200/60 pb-2">{t.licenses}</h3>
                    <div className="flex text-center divide-x divide-slate-200">
                        <div onClick={() => navigateTo('profile', { tab: 'licenses' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-brand-600 font-serif drop-shadow-sm">{licenseStats.active}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.activas}</div>
                        </div>
                        <div onClick={() => navigateTo('profile', { tab: 'licenses' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-yellow-500 font-serif drop-shadow-sm">{licenseStats.expiring}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.expiring}</div>
                        </div>
                        <div onClick={() => navigateTo('profile', { tab: 'licenses' })} className="flex-1 flex flex-col items-center px-2 transform hover:-translate-y-1 transition-transform cursor-pointer">
                            <div className="text-4xl font-bold text-red-500 font-serif drop-shadow-sm">{licenseStats.expired}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase mt-2 tracking-widest">{t.expired}</div>
                        </div>
                    </div>
                </div>
            </div>

            {/* Card 6 — News (agent) / Needed Actions (agency) */}
            <div className="lg:col-span-3 flex flex-col">
                {!isAgency ? (
                    <NestedCard title={t.news} className="flex-1">
                        <NewsCard t={t} />
                    ...
                    <NestedCard title={t.neededActions} className="flex-1 flex flex-col">
                        <div className="flex flex-col gap-4 flex-1 justify-center">
                            <div onClick={() => navigateTo('contracts')} className="flex items-center gap-4 p-4 rounded-2xl bg-red-50 border border-red-100 cursor-pointer hover:bg-red-100 transition-colors group">
                                ...
                                <div>
                                    <div className="text-3xl font-bold text-red-500 font-serif leading-none">23</div>
                                    <div className="text-xs text-red-400 font-semibold mt-1">{t.unreadMessages}</div>
                                </div>
                            </div>
                            <div onClick={() => navigateTo('contracts')} className="flex items-center gap-4 p-4 rounded-2xl bg-yellow-50 border border-yellow-100 cursor-pointer hover:bg-yellow-100 transition-colors group">
                                <div className="w-12 h-12 rounded-xl bg-yellow-100 flex items-center justify-center shrink-0 group-hover:bg-yellow-200 transition-colors">
                                    <Icons.Clock />
                                </div>
                                <div>
                                    <div className="text-3xl font-bold text-yellow-500 font-serif leading-none">47</div>
    ...
    <div>
        ...
        <div className="relative">
            <select
                ...
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-700 bg-white focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all appearance-none cursor-pointer pr-10"
            >
    ...
    return ReactDOM.createPortal(
        <>
            ...
            <div className="fixed inset-0 flex items-center justify-center z-[201] pointer-events-none px-4">
                <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md pointer-events-auto fade-in">
                    <h2 className="text-2xl font-bold text-slate-800 mb-1">{t.contractRequest}</h2>
                    ...
                    <div className="flex justify-end items-center gap-5 mt-8">
                        ...
                        <button className="bg-blue-600 hover:bg-blue-700 text-white px-7 py-2.5 rounded-xl font-semibold transition-colors shadow-sm text-sm">{t.submit}</button>
                    </div>
                </div>
            </div>
        </>,
...
const ColDropdown = ({ col, open, onToggle, onSelect, activeFilter }) => {
    ...
    const menu = open ? ReactDOM.createPortal(
        <div
            ...
            className="bg-white rounded-2xl shadow-lg border border-slate-100 py-1.5 min-w-[150px]"
    ...
    return (
        <th className="p-5 font-medium">
            ...
            >
                <span>{col.label}</span>
                <span className={`flex items-center justify-center w-4 h-4 rounded bg-slate-100 group-hover:bg-brand-100 transition-colors ${open ? 'bg-brand-100' : ''} ${activeFilter ? 'bg-brand-200' : ''}`}>
...
const AiSearchBar = ({ placeholder }) => {
    ...
    return (
        <div className={`flex items-center gap-3 w-full mb-5 px-4 py-3 rounded-2xl border transition-all duration-200 bg-white
            ...
            />
            <span className="flex-shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-50 text-blue-500 border border-blue-200">
                {t.aiSearchLabel}
            </span>
        </div>
    );
...
const ContractsView = ({ navigateTo, initialTab }) => {
    ...
    const allLicenses = [
        { state: 'TX', type: 'Health and Life', expiry: 'Apr-08-2028', status: 'Active' },
        { state: 'FL', type: 'Health Only',     expiry: 'Dec-15-2026', status: 'Expiring Soon' },
        { state: 'NY', type: 'Life',            expiry: 'Jan-01-2025', status: 'Expired' },
    ];
    ...
    const allData = [
        { id: '#1349', agency: 'Cardali', agent: 'Nacho Molano', carrier: 'Anthem Blue Cross',  type: 'Medicare Advantage',        state: 'TX', status: 'In Progress',       date: '12 May 2026' },
        { id: '#1350', agency: 'Cardali', agent: 'Ivan Diaz',    carrier: 'Aetna',              type: 'Health (ACA / Marketplace)', state: 'FL', status: 'Approved',          date: '10 May 2026' },
        { id: '#1351', agency: 'Direct',  agent: 'Maria Santos', carrier: 'Cigna',              type: 'Life Insurance',             state: 'NY', status: 'Release Requested', date: '08 May 2026' },
        { id: '#1352', agency: 'Cardali', agent: 'Ivan Diaz',    carrier: 'UnitedHealthcare',   type: 'Medicare Advantage',        state: 'TX', status: 'Approved',          date: '01 May 2026' },
        { id: '#1353', agency: 'Cardali', agent: 'Pedro Gil',    carrier: 'Humana',             type: 'Health (ACA / Marketplace)', state: 'FL', status: 'New',               date: '28 Apr 2026' },
        { id: '#1354', agency: 'Direct',  agent: 'Carlos Cruz',  carrier: 'Ambetter',           type: 'Health (ACA / Marketplace)', state: 'TX', status: 'Rejected',          date: '25 Apr 2026' },
    ];
    ...
    const colSpanCount = isAgency ? 7 : 7;
    ...
    return (
        <div className="fade-in">
            ...
            <div className="flex justify-between items-center mb-5">
                ...
                <button onClick={() => setShowModal(true)} className="bg-slate-800 text-white px-5 py-2.5 rounded-full font-medium hover:bg-slate-700 transition-colors shadow-soft">
                    {t.requestNew}
                </button>
            </div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-5">
                {[
                    { label: t.approved,         count: isAgency ? 312  : 8,  color: 'text-brand-600',  status: 'Approved'          },
                    { label: t.inProgress,       count: isAgency ? 1571 : 24, color: 'text-blue-600',   status: 'In Progress'       },
                    { label: t.rejected,         count: isAgency ? 3646 : 5,  color: 'text-red-500',    status: 'Rejected'          },
                    { label: t.newStatus,        count: isAgency ? 35   : 3,  color: 'text-slate-700',  status: 'New'               },
                    { label: t.releaseRequested, count: isAgency ? 13   : 2,  color: 'text-yellow-600', status: 'Release Requested' },
                    { label: t.releaseRequired,  count: isAgency ? 57   : 4,  color: 'text-orange-500', status: 'Release Required'  },
                    { label: t.expired,          count: isAgency ? 42   : 6,  color: 'text-slate-400',  status: 'Expired'           },
                ].map(kpi => (
                    <button key={kpi.label}
                        onClick={() => toggleStatus(kpi.status)}
                        className={`text-center py-4 px-3 rounded-3xl shadow-soft transition-all duration-200 hover:shadow-soft-hover
                            ${activeStatuses.has(kpi.status)
                                ? 'bg-white ring-2 ring-brand-400 scale-105'
                                : 'bg-white hover:scale-105'}`}>
                        <p className={`text-3xl font-bold font-serif ${kpi.color}`}>{kpi.count.toLocaleString()}</p>
            ...
            <div className="flex items-center gap-3 mb-5 flex-wrap">
                ...
                <div className={`flex items-center gap-2 border rounded-full pl-3 pr-4 py-2 bg-white transition-colors
                            ...
                            {activeStatuses.size > 0 && (
                                <div className="flex items-center gap-2">
                                    {[...activeStatuses].map(s => (
                                        <span key={s} className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-100 text-brand-700 text-xs font-semibold">
                                            {s}
                                ...
                                <tbody>
                                    {visibleRows.map((row, i) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-5 text-slate-600">{row.state}</td>
                                            <td className="p-5">
                ...
                <div className="w-80 flex-shrink-0">
                    {isAgency ? (
                        <Card className="border border-blue-200">
                            <h2 className="text-3xl font-bold text-slate-800 mb-5">{t.licencias}</h2>

                            <div className="mb-5">
                                <h3 className="text-xl font-semibold text-slate-700 mb-3">{t.licAgency}</h3>
                                <div className="space-y-2">
                                    {[
                                        { label: t.active,   count: 12 },
                                        { label: t.expired,  count: 2  },
                                        { label: t.expiring, count: 4  },
                                    ].map(row => (
                                        <div key={row.label} className="flex justify-between items-center">
                                            <span className="text-slate-600">{row.label}</span>
                                            <span className="text-2xl font-bold text-slate-800 w-12 text-right">{String(row.count).padStart(2,'0')}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="mb-6">
                                <h3 className="text-xl font-semibold text-slate-700 mb-3">{t.licPrincipal}</h3>
                                <div className="space-y-2">
                                    {[
                                        { label: t.active,   count: 28 },
                                        { label: t.expired,  count: 5  },
                                        { label: t.expiring, count: 7  },
                                    ].map(row => (
                                        <div key={row.label} className="flex justify-between items-center">
                                            <span className="text-slate-600">{row.label}</span>
                                            <span className="text-2xl font-bold text-slate-800 w-12 text-right">{String(row.count).padStart(2,'0')}</span>
                            ...
                            <div className="flex justify-end">
                                <button
                                    onClick={() => navigateTo('profile', { tab: 'licenses' })}
                                    className="px-4 py-1.5 rounded-full border border-slate-300 text-sm text-slate-600 hover:border-brand-400 hover:text-brand-600 transition-colors"
                                >
                                    {t.details}
                                </button>
                            </div>
                        ...
                        <Card className="bg-gradient-to-b from-white to-slate-50">
                            <h3 className="font-bold text-slate-800 mb-3">{t.licencias}</h3>
                            <div className="flex gap-1.5 mb-4">
                                {[
                                    ...
                                    <button key={tab.key} onClick={() => setLicenseFilter(tab.key)}
                                        className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors whitespace-nowrap
                                            ${licenseFilter === tab.key ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                                        {tab.label}
                                    </button>
                                ))}
                            </div>
                            <div className="space-y-3">
                                {allLicenses.filter(l => l.status === licenseFilter).map((lic, i) => {
                                    ...
                                    const accentBorder = isActive ? 'border-l-brand-500' : isExpiring ? 'border-l-yellow-400' : 'border-l-red-400';
                                    const badgeVariant = isActive ? 'green' : isExpiring ? 'yellow' : 'red';
                                    const badgeLabel   = isActive ? t.activas : isExpiring ? t.proxVencer : t.expiradas;
                                    return (
                                        <div key={i} onClick={() => navigateTo('profile', { tab: 'licenses' })}
                                            className={`p-3 border border-slate-100 bg-white rounded-xl shadow-sm border-l-4 ${accentBorder} cursor-pointer hover:shadow-soft transition-all`}>
                                            <div className="flex justify-between items-start mb-1">
                                                <span className="font-bold text-slate-800">{lic.state}</span>
                                                <Badge variant={badgeVariant}>{badgeLabel}</Badge>
                                            </div>
                                            <p className="text-sm text-slate-600 font-medium">{lic.type}</p>
                                            {lic.status !== 'Expired' && (
                                                <p className="text-xs text-slate-400 mt-1.5">⏱ {lic.expiry}</p>
                                            )}
                                        </div>
                                    );
                                })}
                                {allLicenses.filter(l => l.status === licenseFilter).length === 0 && (
                                    <p className="text-sm text-slate-400 text-center py-4">{t.sinLicencias}</p>
                                )}
                            </div>
                        </Card>
                    )}
...
const AgencyRequirementsPanel = () => {
    ...
    const ReqRow = ({ req }) => (
        <div className={`flex items-start gap-4 p-4 rounded-2xl border transition-all ${req.completed ? 'bg-slate-50 border-slate-100 opacity-70' : 'bg-white border-slate-200 shadow-sm'}`}>
            <button
                ...
                <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        {req.agentUploaded && !req.completed && (
                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">Agent uploaded</span>
                        )}
                        {req.due && (
        ...
        <div className="space-y-6 fade-in">
            ...
            <div className="flex items-center gap-4">
                ...
                <button
                    ...
                    className="px-4 py-2 rounded-full bg-slate-800 text-white text-sm font-medium hover:bg-slate-700 transition-colors whitespace-nowrap"
                >
                    + Add Requirement
                </button>
            </div>
            ...
            {showNewReq && (
                <div className="bg-brand-50 border border-brand-200 rounded-2xl p-5 space-y-3 fade-in">
                    ...
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowNewReq(false)}
                            className="px-4 py-2 rounded-full text-sm text-slate-500 hover:text-slate-700 transition-colors">{t.cancel}</button>
                        <button onClick={addReq}
                            className="px-4 py-2 rounded-full bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 transition-colors">{t.submit}</button>
                    </div>
                </div>
            )}
    ...
    return (
        <div className="fade-in">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-1/3 h-fit flex-shrink-0 bg-white rounded-3xl shadow-soft p-6">
                    <h2 className="text-2xl font-bold mb-4 text-slate-700">Contrato #1349</h2>
                ...
                <div className="flex-1 flex flex-col">
                    <div className="flex gap-4 mb-6 border-b border-slate-200">
                        ...
                        >
                            {t.messages} <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">1</span>
                        </button>
                    </div>
                        ...
                        <div className="space-y-8 fade-in">
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div className="bg-orange-50 border-2 border-dashed border-orange-200 rounded-2xl p-5 hover:bg-orange-100 transition-colors flex flex-col justify-between group">
                                        <div>
                                            <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <button className="w-full bg-white text-orange-600 border border-orange-200 py-2 rounded-xl font-medium shadow-sm hover:shadow group-hover:bg-orange-50 flex justify-center items-center gap-2 transition-all">
                                                <Icons.Upload /> Subir Archivo
                                            </button>
                                        </div>
                                    </div>

                                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between group">
                                        <div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <button onClick={() => navigateTo('profile')} className="w-full bg-slate-800 text-white py-2 rounded-xl font-medium shadow-soft hover:bg-slate-700 flex justify-center items-center gap-2 transition-all">
                                    ...
                                    {/* AI Reviewing card */}
                                    <div className="rounded-2xl p-5 border-2 bg-blue-50/60 ai-pulse-border flex flex-col justify-between">
                                        <div>
                                            <div className="flex justify-between items-start mb-2">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex items-center gap-4 opacity-80">
                                    ...
                                    <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex items-center gap-4 opacity-70">
                                        <div className="w-12 h-12 rounded-full bg-green-50 flex items-center justify-center text-green-600">
                                            <Icons.CheckCircle />
                                        </div>
                                        <div>
                        ...
                        <div className="fade-in space-y-6">
                            ...
                            <Card>
                                <div className="flex items-center gap-5 mb-6">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-bold text-slate-800">Nacho Molano</h2>
                        ...
                        <Card className="flex-1 flex flex-col h-[500px] p-0 overflow-hidden fade-in border border-slate-200">
                            ...
                            <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-white">
                                <div className="flex justify-start">
                                    <div className="bg-slate-100 text-slate-800 px-4 py-3 rounded-2xl rounded-tl-sm max-w-[80%] shadow-sm">
                                ...
                                <div className="flex justify-end">
                                    <div className="bg-brand-500 text-white px-4 py-3 rounded-2xl rounded-tr-sm max-w-[80%] shadow-sm">
                            ...
                            <div className="p-4 bg-white border-t border-slate-100">
                                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-full p-2">
...
const ProfileView = ({ initialTab }) => {
    ...
    const allLicenses = [
        { state: 'Texas (TX)',    type: 'Health and Life', number: 'TX-998877', expiry: '08 Abr 2028', status: 'Active',        aiVerified: true  },
        { state: 'New York (NY)', type: 'Life',            number: 'NY-445566', expiry: '01 Ene 2027', status: 'Active',        aiVerified: false },
        { state: 'Florida (FL)',  type: 'Health',          number: 'FL-112233', expiry: '15 Dic 2026', status: 'Expiring Soon', aiVerified: false },
        { state: 'Georgia (GA)',  type: 'Health and Life', number: 'GA-778899', expiry: '03 Mar 2025', status: 'Expired',       aiVerified: false },
    ];
    const menuItems = [
        ...
        <div className="fade-in flex flex-col md:flex-row gap-8">
                ...
                <div className="sticky top-8 space-y-1">
                    {menuItems.map(item => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl font-medium transition-all duration-300 ${activeTab === item.id ? 'bg-white text-brand-600 shadow-soft scale-105' : 'text-slate-500 hover:bg-white/50 hover:text-slate-800'}`}
                        >
                            {item.icon}
                            {item.label}
                        </button>
            ...
            <div className="flex-1">
                    ...
                    <Card className="fade-in relative overflow-hidden">
                        ...
                        <div className="relative z-10 pt-36 pb-6 px-6">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h2 className="text-3xl font-bold text-slate-800">Nacho Molano</h2>
                                    <p className="text-slate-500">{isAgency ? 'Agency Manager' : 'Cardali Agent'}</p>
                                </div>
                                <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-5 py-2 rounded-xl font-medium transition-colors">
                                    Editar Perfil
                                </button>
                            </div>
                        </div>
                ...
                {activeTab === 'documents' && (
                    <div className="fade-in space-y-8">
                        <div>
                            <h3 className="text-xl font-bold text-slate-800 mb-4">Documentos Básicos</h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <Card className="flex flex-col justify-between border-2 border-red-100 bg-red-50/50">
                                    ...
                                    <button className="mt-6 w-full bg-white text-slate-700 border border-slate-200 py-2.5 rounded-xl font-medium shadow-sm hover:shadow hover:border-brand-300 transition-all flex items-center justify-center gap-2">
                                ...
                                <Card className="flex flex-col justify-between bg-white border border-slate-100">
                                    ...
                                    <button className="mt-6 w-full bg-slate-50 text-slate-600 py-2.5 rounded-xl font-medium hover:bg-slate-100 transition-colors">
                                        Actualizar Archivo
                                    </button>
                                </Card>
                ...
                {activeTab === 'licenses' && (
                    <Card className="fade-in">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-slate-800">{t.licensesNav}</h3>
                        ...
                        <div className="space-y-4">
                            {allLicenses.filter(l => l.status === licenseFilter).map((lic, i) => {
                                ...
                                return (
                                    <div key={i} className="flex flex-col md:flex-row md:items-center justify-between p-5 border border-slate-100 rounded-2xl hover:border-brand-200 transition-colors bg-slate-50/50">
                        ...
                        <button className="mt-6 w-full border-2 border-dashed border-slate-200 text-slate-500 py-4 rounded-2xl font-bold hover:bg-slate-50 hover:text-brand-600 hover:border-brand-300 transition-all flex justify-center items-center gap-2">
...
const NewPolicyModal = ({ onClose }) => {
    ...
    const inputCls = "w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-700 bg-white focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all text-sm";
    ...
    return ReactDOM.createPortal(
        <>
            ...
            <div className="fixed inset-0 flex items-center justify-center z-[201] pointer-events-none px-4">
                <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md pointer-events-auto fade-in">
                    <h2 className="text-2xl font-bold text-slate-800 mb-1">{t.addPolicy}</h2>
                    <div className="space-y-5 mt-7">
                        <div>
...
const PolicyFilterPill = ({ label, value, onChange, options }) => (
    <div className="relative">
        <select
            value={value}
            onChange={e => onChange(e.target.value)}
            className={`appearance-none border rounded-full pl-4 pr-8 py-2 text-sm bg-white cursor-pointer focus:outline-none transition-colors
...
const PoliciesView = ({ navigateTo, initialTab }) => {
    ...
    return (
        <div className="fade-in">
            ...
            <div className="flex justify-between items-center mb-5">
                <h1 className="text-5xl font-bold text-slate-800 tracking-tight">{isAgency ? t.agencyPolicies : t.myPolicies}</h1>
                <button onClick={() => setShowModal(true)} className="bg-slate-800 text-white px-5 py-2.5 rounded-full font-medium hover:bg-slate-700 transition-colors shadow-soft">
                    {t.newPolicy}
                </button>
            </div>
                                ...
                                <th className="p-5 font-semibold text-slate-500 text-sm">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <input type="checkbox" onClick={e => e.stopPropagation()}
                                            className="w-4 h-4 rounded border-slate-300 cursor-pointer accent-brand-500" />
                                    </div>
                                </th>
                            </tr>
                        </thead>
                                    ...
                                    {isAgency && (
                                        <td className="p-5 align-top">
                                            ...
                                            <div className="flex flex-col items-start gap-1 mt-2">
                                                ...
                                                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${refBadgeClass}`}>{refBadgeLabel}</span>
                                            </div>
                                        </td>
                                    )}
                                    <td className="p-5 align-top">
...
const PolicyDetailView = ({ navigateTo, initialTab }) => {
    ...
    return (
        <div className="fade-in">
            ...
            {/* Progress bar */}
            <div className="flex mb-6 rounded-2xl overflow-hidden shadow-soft">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                {/* Sidebar */}
                <div className="w-full md:w-72 flex-shrink-0 bg-white rounded-3xl shadow-soft p-5 h-fit">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-72 flex-shrink-0 space-y-4">
                    ...
                    {agent.pendingReqs.length > 0 && (
                        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:bg-orange-100 transition-colors"
                        ...
                        <div className="space-y-6 fade-in">
                            ...
                            <Card>
                                <h3 className="text-base font-bold text-slate-700 mb-4 flex items-center gap-2">
                                    ...
                                    {t.pendingReqs2}
                                    {agent.pendingReqs.length > 0 && (
                                        <span className="ml-1 text-xs bg-orange-100 text-orange-700 font-semibold px-2 py-0.5 rounded-full">{agent.pendingReqs.length}</span>
                                    )}
                                </h3>
                                {agent.pendingReqs.length === 0 ? (
...
const MyAgencyView = ({ navigateTo }) => {
    ...
    return (
        <div className="fade-in">
            <div className="flex justify-between items-center mb-5">
                <h1 className="text-5xl font-bold text-slate-800 tracking-tight">{t.myAgency}</h1>
                <button className="bg-slate-800 text-white px-5 py-2.5 rounded-full font-medium hover:bg-slate-700 transition-colors shadow-soft">
                    + Invite Agent
                </button>
            </div>
...
const AidaChat = () => {
    ...
    return ReactDOM.createPortal(
        <div className="fixed bottom-6 right-6 z-[1000] flex flex-col items-end gap-3">
            {/* Chat panel */}
            {open && (
                <div className="w-80 bg-white rounded-3xl shadow-[0_20px_60px_-10px_rgba(0,0,0,0.2)] border border-slate-100 flex flex-col overflow-hidden fade-in"
                    style={{ height: '480px' }}>
                    {/* Header */}
                        ...
                        <div className="px-3 pb-2 flex gap-1.5 flex-wrap flex-shrink-0">
                            {AIDA_SUGGESTIONS.map(s => (
                                <button key={s} onClick={() => send(s)}
                                    className="px-2.5 py-1 rounded-full bg-brand-50 text-brand-700 text-[10px] font-medium border border-brand-200 hover:bg-brand-100 transition-colors whitespace-nowrap">
                                    {s}
                                </button>
                            ))}
                        </div>
...
const App = () => {
    ...
    return (
        ...
        <div className="min-h-screen flex flex-col">
            <nav className="p-6 pb-8 sticky top-0 z-50 pointer-events-none">
                <div className="flex justify-between items-center bg-white/80 backdrop-blur-md p-2 rounded-full shadow-soft pointer-events-auto border border-white/50">
                    ...
                    <div className="flex items-center pl-2">
                        ...
                        <div className="border-l border-slate-200/60 pl-4">
                            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 hidden md:block">
                    ...
                    <div className="flex-1 flex justify-center items-center gap-2 px-4">
                        {[
                            ...
                            <button
                                ...
                                className={`px-6 py-2.5 rounded-full font-medium text-sm transition-all duration-300 ease-in-out whitespace-nowrap
                        ...
                        <div className="relative" onClick={e => e.stopPropagation()}>
                            ...
                            {showNotifications && (
                                <div className="absolute left-1/2 -translate-x-1/2 top-full mt-3 w-80 bg-white rounded-2xl shadow-lg border border-slate-100 z-50 overflow-hidden fade-in">
                                    <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center">
                                        <h4 className="font-bold text-slate-800 text-sm">Mensajes recientes</h4>
                                        {unreadCount > 0 && (
                                            <span className="text-xs bg-red-100 text-red-600 font-semibold px-2 py-0.5 rounded-full">{unreadCount} nuevos</span>
                                        )}
                                    </div>
                                    <div>
                                        {NOTIFICATIONS.map(n => (
                                                ...
                                                <div className="flex gap-3 items-start">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex justify-between items-baseline gap-2">
                                                            ...
                                                            <span className="text-[10px] text-slate-400 shrink-0 font-serif">{n.time}</span>
                                                        </div>
                    ...
                    <div className="flex items-center gap-2 pr-1">
                        ...
                        <button
                            onClick={toggleLang}
                            className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-slate-100 transition-colors overflow-hidden"
                        ...
                        >
                            ...
                            <div className="w-8 h-8 rounded-full bg-slate-200 overflow-hidden border-2 border-white shadow-sm flex items-center justify-center text-sm">
                                👨🏽‍💼
                            </div>
                        </button>
                    </div>
